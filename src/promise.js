...
'use strict';
    ...
    // class style promise .
    // the essential of promise is pass a trigger to callback ,
    // and use that trigger to call cached execs( catched by then)
...
function  tsChain2() {
  ...
  p.then(rt => {
    log(' then 2');
    return c2log(`2~22~ ${rt}`, 500);
  }).then(rt => {
    log(' then 3');
    return c2log(`3~~ ${rt}`, 3000);
  }).then(rt => {
    log(' then 4');
    return c2log(`4~~ ${rt}`, 1200);
  }).then(rt => {
    log(' then 5');
    return c2log(`4~~ ${rt}`, 1200);
  });
